...
/**
 * @brief Load the header containing a matcher's packet value.
 *
 * Most packet matchers use their protocol layer directly. META ports are
 * backed by the L4 header but accept both TCP and UDP, so they validate that
 * the packet has either protocol before loading the common L4 header.
 */
static int _bf_matcher_pkt_load_header(struct bf_program *program,
                                       ...
                                       int reg)
{
    ...
    case BF_MATCHER_META_DPORT:
        ...
        return 0;
    default:
        return bf_stub_load_header(program, meta, reg);
    }
}
                                         ...
                                         const struct bf_matcher_meta *meta)
{
    ...
    r = _bf_matcher_pkt_load_header(program, bf_matcher_get_type(matcher), meta,
                                    BPF_REG_6);
    if (r)
        return r;
                                        ...
                                        const struct bf_matcher *matcher)
{
    ...
    for (size_t i = 0; i < set->n_comps; ++i) {
        ...
        r = _bf_matcher_pkt_load_header(program, type, meta, BPF_REG_6);
        if (r)
            return bf_err_r(r, "failed to load protocol header into BPF_REG_6");

...
static void hook_to_str(void **state)
{
    ...
    assert_string_equal(bf_hook_to_str(BF_HOOK_CGROUP_CONNECT4),
                        "BF_HOOK_CGROUP_CONNECT4");
    assert_string_equal(bf_hook_to_str(BF_HOOK_CGROUP_CONNECT6),
                        "BF_HOOK_CGROUP_CONNECT6");
}
...
static void hook_to_flavor(void **state)
{
    ...
    assert_int_equal(bf_hook_to_flavor(BF_HOOK_CGROUP_EGRESS),
                     BF_FLAVOR_CGROUP);
    assert_int_equal(bf_hook_to_flavor(BF_HOOK_CGROUP_CONNECT4),
                     BF_FLAVOR_CGROUP_SOCK_ADDR);
    assert_int_equal(bf_hook_to_flavor(BF_HOOK_CGROUP_CONNECT6),
                     BF_FLAVOR_CGROUP_SOCK_ADDR);
}
...
static void hook_to_bpf_attach_type(void **state)
{
    ...
    assert_int_equal(bf_hook_to_bpf_attach_type(BF_HOOK_TC_EGRESS),
                     BF_BPF_TCX_ENGRESS);
    assert_int_equal(bf_hook_to_bpf_attach_type(BF_HOOK_CGROUP_CONNECT4),
                     BF_BPF_CGROUP_INET4_CONNECT);
    assert_int_equal(bf_hook_to_bpf_attach_type(BF_HOOK_CGROUP_CONNECT6),
...
static void hook_to_bpf_prog_type(void **state)
{
    ...
    assert_int_equal(bf_hook_to_bpf_prog_type(BF_HOOK_CGROUP_CONNECT4),
                     BF_BPF_PROG_TYPE_CGROUP_SOCK_ADDR);
    assert_int_equal(bf_hook_to_bpf_prog_type(BF_HOOK_CGROUP_CONNECT6),
                     BF_BPF_PROG_TYPE_CGROUP_SOCK_ADDR);
}
...
static void hookopts_validate_cgroup_sock_addr(void **state)
{
    ...
    // cgroup_sock_addr requires cgpath
    {
        _free_bf_hookopts_ struct bf_hookopts *hookopts = NULL;
        assert_ok(bf_hookopts_new(&hookopts));
        assert_err(bf_hookopts_validate(hookopts, BF_HOOK_CGROUP_CONNECT4));
        assert_err(bf_hookopts_validate(hookopts, BF_HOOK_CGROUP_CONNECT6));
    }
    ...
    // With cgpath, should be valid
    {
        ...
        assert_ok(bf_hookopts_new(&hookopts));
        assert_ok(bf_hookopts_parse_opt(hookopts, opt));
        assert_ok(bf_hookopts_validate(hookopts, BF_HOOK_CGROUP_CONNECT4));
        assert_ok(bf_hookopts_validate(hookopts, BF_HOOK_CGROUP_CONNECT6));
    }

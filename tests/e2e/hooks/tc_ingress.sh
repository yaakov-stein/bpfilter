...
hook_extra_setup() {
    ...
    # Pre-populate NDP neighbor tables to avoid resolution delay on first ping.
...
hook_send_icmp4()  { ping -c 1 -W 0.1 "${NS_IP_ADDR}"; }
...
hook_send_icmp6()  { ping -6 -c 1 -W 0.1 "${NS_IP6_ADDR}"; }
